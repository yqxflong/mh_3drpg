...
PROJECT_NAME="mhj"
TARGET_PATH="${BUILD_PATH}/Client/Temp/Client-iOS/$PROJECT_NAME"
TARGET_NAME="Unity-iPhone"
CONFIGURATION="Debug"
EXPORT_OPTIONS_PLIST="$TOOLS_PATH/exportOptions.plist"
...
echo "Start Automation.BuildiOSDeviceDev ..."
"$UNITY" -projectPath "$PROJECT_PATH" -executeMethod Automation.BuildiOSDeviceDev -batchmode -quit -logFile "$LOG_FILE"
[ $? -ne 0 ] && echo "Automation.BuildiOSDeviceDev failed" && exit
[ ! -d "$TARGET_PATH" ] && echo "$TARGET_PATH not exists" && exit
echo "Automation.BuildiOSDeviceDev done"
