...
LIB_DEST_DIR="$BUILD_PATH/opencore-amr-dest-lib"
HEADER_DEST_DIR="$BUILD_PATH/opencore-amr-dest-include"
...
configure_make()
{
   ...
   CPPFLAGS=" -arch ${ARCH} ${MIN_FLAG} ${EXTRA_FLAG} -isysroot ${SDK_PATH} -I${DEPENDENCIES_DIR_HEAD} " &> "${LOG_FILE}"
...
PROJECT_LIBS=("libopencore-amrnb.a" "libopencore-amrwb.a")
for ((i=0; i < ${#LIBS[@]}; i++));
do
