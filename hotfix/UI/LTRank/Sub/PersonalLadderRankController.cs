...
{
    public class PersonalLadderRankController : CommonRankPageController<PersonalLadderRankItemData>
    {
        protected override void UpdateUI(ArrayList array)
        {
            rankdatas = new List<PersonalLadderRankItemData>();
            long localPlayerId = LoginManager.Instance.LocalUserId.Value;
            m_localPlayerRankData = null;
            for (int i = 0; i < array.Count; i++)
            {
                var data = new PersonalLadderRankItemData(array[i] as Hashtable, i);
            ...
            //rankdatas.Sort(new PersonalLadderRankItemDataComparer());
            ...
            isDataReady = true;
            defaultocalPlayerRankLabelData = EB.Localizer.GetString("ID_GOT_STAGE");
            UpdateInfo();
        }
        ...
        protected override void InitText()
        {
            ...
            //玩家段位积分
            LTUIUtil.SetText(RightText, Localizer.GetString("ID_uifont_in_LTRankListHud_Score_3"));
        }

        public override void Awake()
        {
            base.Awake();
            m_PageDataPath = "rank.personal.ladder";
        }
    }
}
