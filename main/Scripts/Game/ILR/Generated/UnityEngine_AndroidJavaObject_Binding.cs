...
namespace ILRuntime.Runtime.Generated
{
    unsafe class UnityEngine_AndroidJavaObject_Binding
    {
        public static void Register(ILRuntime.Runtime.Enviorment.AppDomain app)
        {
            ...
            args = new Type[]{typeof(UnityEngine.AndroidJavaObject)};
            if (genericMethods.TryGetValue("GetStatic", out lst))
            {
                foreach(var m in lst)
                {
                    if(m.MatchGenericParameters(args, typeof(UnityEngine.AndroidJavaObject), typeof(System.String)))
                    {
                        method = m.MakeGenericMethod(args);
                        app.RegisterCLRMethodRedirection(method, GetStatic_0);
            ...
            method = methods.Where(t => t.Name.Equals("CallStatic") && t.CheckMethodParams(args)).Single();
            app.RegisterCLRMethodRedirection(method, CallStatic_1);


        }


        static StackObject* GetStatic_0(ILIntepreter __intp, StackObject* __esp, IList<object> __mStack, CLRMethod __method, bool isNewObj)
        {
            ...
            var result_of_this_method = instance_of_this_method.GetStatic<UnityEngine.AndroidJavaObject>(@fieldName);
