...
public partial class AstarPath : MonoBehaviour {
...
#line hidden
	...
	private static void AstarLog (string s) {
		if (System.Object.ReferenceEquals(active, null)) {
			EB.Debug.Log("No AstarPath object was found : {0}",s);
			return;
		}
...
#line default
	 ...
	 */
	private void Update () {
        if (GameEngine.Instance != null && !GameEngine.Instance.IsTimeToRootScene)
        {
            return;
        }
			...
			if (s.order == GraphUpdateOrder.FloodFill) {
				...
				if (threading == GraphUpdateThreading.SeparateAndUnityInit) {
					try {
						...
						EB.Debug.LogError("Error while initializing GraphUpdates\n{0}" , e);
					}
				}
				try {
					s.graph.UpdateArea(s.obj);
				} catch (System.Exception e) {
					EB.Debug.LogError("Error while updating graphs\n{0}",e);
				}
			}
#if !UNITY_WEBGL
		}
		...
		while (!astar.pathQueue.IsTerminating) {
			...
			while (graphUpdateQueueAsync.Count > 0) {
				...
				try {
					...
					EB.Debug.LogError("Exception while updating graphs:\n{0}",e);
				}
			}
		...
		if (threads != null) {
			for (int i = 0; i < threads.Length; i++) {
				...
				if (!threads[i].Join(50)) {
					EB.Debug.LogError("Could not terminate pathfinding thread[{0}] in 50ms, trying Thread.Abort" ,i);
					threads[i].Abort();
				}
#endif
			}
...
#region ScanMethods
	...
	public void FloodFill (GraphNode seed, uint area) {
		if (area > GraphNode.MaxAreaIndex) {
			EB.Debug.LogError("Too high area index - The maximum area index is {0}", GraphNode.MaxAreaIndex);
			return;
		}
	...
	public void FloodFill () {
		...
		if (warnAboutAreas) {
			EB.Debug.LogError("Too many areas - The maximum number of areas is {0}. Try raising the A* Inspector -> Settings -> Min Area Size value. Enable the optimization ASTAR_MORE_AREAS under the Optimizations tab.", GraphNode.MaxAreaIndex);
		}
	 ...
	 *EB.Debug.Log ("Scanning... " + progress.description + " - " + (progress.progress*100).ToString ("0") + "%");
	 * } \endcode
	 * \see Scan
	 */
...
#region MainThreads
	...
	private static void CalculatePathsThreaded (System.Object _threadInfo) {
		...
		try {
			...
			EB.Debug.LogError("Arguments to pathfinding threads must be of type ThreadStartInfo\n{0}",e);
...
#endif
		{
			if (astar.logPathResults == PathLog.Heavy)
				EB.Debug.LogWarning("Shutting down pathfinding thread #{0}",threadInfo.threadIndex);
			return;
		}
		Debug.LogException(e);
		Debug.LogError("Unhandled exception during pathfinding. Terminating.");
