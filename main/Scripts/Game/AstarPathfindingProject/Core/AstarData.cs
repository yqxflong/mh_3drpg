	...
	public class AstarData {
		...
		public void DeserializeGraphs (byte[] bytes) {
			...
			try {
				...
				EB.Debug.LogWarning("Caught exception while deserializing data.\n{0}",e);
				data_backup = bytes;
			}
		}
		...
		public NavGraph CreateGraph (string type) {
			EB.Debug.Log("Creating Graph of type '{0}'",type);
			...
			EB.Debug.LogError("Graph type ({0}) wasn't found" , type);
			return null;
		}
		...
		public NavGraph AddGraph (string type) {
			...
			if (graph == null) {
				EB.Debug.LogError("No NavGraph of type '{0}' could be found", type);
				return null;
			}
		...
		public NavGraph AddGraph (System.Type type) {
			...
			if (graph == null) {
				EB.Debug.LogError("No NavGraph of type '{0}' could be found,{1}  graph types are avaliable",type,graphTypes.Length);
				return null;
			}
