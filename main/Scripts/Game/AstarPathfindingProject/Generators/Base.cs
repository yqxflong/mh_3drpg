...
namespace Pathfinding {
	...
	public abstract class NavGraph {
		 ...
		 *    EB.Debug.Log ("I found a node at position " + (Vector3)node.Position);
		 *     return true;
		 * });
		 * \endcode
		 */
		...
		public RaycastHit[] CheckHeightAll (Vector3 position) {
			...
			while (true) {
				...
				if (hit.point != prevHit || hits.Count == 0) {
					...
					if (numberSame > 10) {
						EB.Debug.LogError("Infinite Loop when raycasting. Please report this error (arongranberg.com)\n{0} : {1}",cpos,prevHit);
						break;
					}
				}
			}
