	...
	public class LayerGridGraph : GridGraph, IUpdatableGraph {
		...
		public override void ScanInternal (OnScanStatus statusCallback) {
			...
			if (layerCount > LevelGridNode.MaxLayerCount) {
				EB.Debug.LogError("Too many layers, a maximum of LevelGridNode.MaxLayerCount are allowed (found {0})",layerCount);
				return;
			}
				...
				do {
					if (layerIndex >= layerCount) {
						if (layerIndex+1 > LevelGridNode.MaxLayerCount) {
							EB.Debug.LogError("Too many layers, a maximum of LevelGridNode.MaxLayerCount are allowed (required {0})",(layerIndex+1));
							return addedNodes;
						}
		...
		public void AddLayers (int count) {
			...
			if (newLayerCount > LevelGridNode.MaxLayerCount) {
				EB.Debug.LogError("Too many layers, a maximum of LevelGridNode.MaxLayerCount are allowed (required {0})",newLayerCount);
				return;
			}
