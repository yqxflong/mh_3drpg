...
public class AstarPathEditor : Editor {
	...
	static void DisableJs () {
		if (System.IO.Directory.Exists(Application.dataPath+"/Plugins/AstarPathfindingProject")) {
			...
			if (error != "") {
				EB.Debug.LogError("Couldn't disable Js - {0}",error);
			} else {
				try {
			...
			EB.Debug.LogWarning("Could not disable JS - Could not find directory '{0}/Plugins/AstarPathfindingProject'" ,Application.dataPath);
		}
	...
	public static TextAsset SaveGraphData (byte[] bytes, TextAsset target = null) {
