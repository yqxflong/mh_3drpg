...
Shader "CmdBuffer/OutLinePostEffect" {

	Properties{
		_MainTex("Base (RGB)", 2D) = "white" {}
	_BlurTex("Blur", 2D) = "white"{}
	_Dir("Dir", Range(-1,1)) = 1
	}
...
#include "UnityCG.cginc"  
	...
	return fixed4(colorBlur.rgb, 0);// - colorMain;
	}
	...
	//blur.a = (blur.r + blur.g + blur.b); //为了RenderTexture有透明
	//fixed4 final = ori + blur * _OutlineStrength;
	//step(i.uv.x, _Dir)目的是外部传0.5时则只有0到0，5的uv.x能渲染，如果时1时则所有都能渲染，为的是渲染自己时不把对方的形象渲染出来
	fixed4 final = blur * _OutlineStrength * step(i.uv.x, _Dir);
	return final;
	}
