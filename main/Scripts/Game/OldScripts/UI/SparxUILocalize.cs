...
public class SparxUILocalize : MonoBehaviour
{
	/// <summary>
	/// Localization key.
	/// </summary>
	...
	public string value
	{
		set
		{
			...
			if (!string.IsNullOrEmpty(value))
			{
				...
				UISymbolLabel slbl = w as UISymbolLabel;
				DynamicUISprite dsp = w as DynamicUISprite;
				...
				{
					// If this is a label used by input, we should localize its default value instead
					UISymbolInput input = NGUITools.FindInParents<UISymbolInput>(slbl.gameObject);
					if (input != null && input.label == slbl) input.defaultText = value;
					else slbl.text = value;
#if UNITY_EDITOR
					if (!Application.isPlaying) NGUITools.SetDirty(slbl);
#endif
				}
				else if (dsp != null)
				{
					UIButton btn = NGUITools.FindInParents<UIButton>(dsp.gameObject);
					if (btn != null && btn.tweenTarget == dsp.gameObject)
						btn.normalSprite = value;

					dsp.spriteName = value;
					dsp.MakePixelPerfect();
#if UNITY_EDITOR
					if (!Application.isPlaying) NGUITools.SetDirty(dsp);
#endif
				}
			}
		}
	...
	void OnLocalize ()
	{
		...
		if (!string.IsNullOrEmpty(key)) value = EB.Localizer.GetString(key);
	}
}
