...
public class UISymbolInput : MonoBehaviour
{
	...
	static public UISymbolInput current;

	/// <summary>
	/// Currently selected input field, if any.
	/// </summary>

	static public UISymbolInput selection;

	/// <summary>
	/// Text label used to display the input's value.
	/// </summary>

	public UISymbolLabel label;
	...
	/// <summary>
	/// Event delegates triggered when the keyboard open.
	/// </summary>
	...
	[System.NonSerialized] protected NGUISymbolText.Alignment mAlignment = NGUISymbolText.Alignment.Left;
	...
	static bool mOpen;
	static protected KeyboardParam mKeyboard { get {
			if (!mOpen)
				return null;

			KeyboardParam param = new KeyboardParam();
			param.text = AndroidKeyboard.Text;
			param.active = AndroidKeyboard.IsVisible;
			param.done = AndroidKeyboard.IsDone;
			param.wasCanceled = AndroidKeyboard.IsCanceled;
			return param;
		}
		set{
			if(value==null)
	...
	static public KeyboardParam Keyboard { get { return mKeyboard; } }

#else
	static private TouchScreenKeyboard mKeyboard;
#endif
	...
	public void Set (string value, bool notify = true)
	{
		...
		if (isSelected && mKeyboard != null && mCached != value)
		{
			...
			// update hidden input
			mobileHiddenInputCompositionString = string.Empty;
			mobileHiddenInputPosition = 0;
			mobileHiddenInputCache = mKeyboard.text;
		}
#endif
		if (mValue != value)
		{
			...
			if (isSelected || label.supportEncoding)
			{
			...
			{
				// correct value, keep in front or end
				label.UpdateNGUIText();
				mValue = NGUISymbolText.StripSymbol(mValue, ref mSelectionStart, characterLimit);
				mSelectionEnd = mSelectionStart;
			}
	...
	protected void Init ()
	{
		if (mDoInit && label != null)
		{
			...
			label.supportEncoding = inputShouldBeHidden;
			selectAllTextOnFocus = !label.supportEncoding;
			mEllipsis = label.overflowEllipsis;

			if (label.alignment == NGUISymbolText.Alignment.Justified)
			{
				label.alignment = NGUISymbolText.Alignment.Left;
				EB.Debug.LogWarning("Input fields using labels with justified alignment are not supported at this time", this);
			}
...
#if !MOBILE
	[System.NonSerialized] UISymbolInputOnGUI mOnGUI;
#endif
...
#if !MOBILE
			if (mOnGUI == null)
				mOnGUI = gameObject.AddComponent<UISymbolInputOnGUI>();
#endif
			OnSelectEvent();
		}
		else
...
#if MOBILE
	private int mobileHiddenInputPosition = 0;
	private string mobileHiddenInputCache = string.Empty;
	private string mobileHiddenInputCompositionString = string.Empty;
#endif
		...
		if (mSelectMe != -1 && mSelectMe != Time.frameCount)
		{
			...
			)
			{
				...
				if (inputShouldBeHidden)
				{
					...
					mCached = mValue;
					mobileHiddenInputPosition = cursorPosition;
					mobileHiddenInputCache = val;
					mobileHiddenInputCompositionString = string.Empty;
#elif UNITY_ANDROID
					int prefix = label.supportEncoding ? mValue.LastIndexOf(']') : -1;
					if (prefix < 0)
					{
						val = mValue;
					...
					{
						val = mValue.Substring(prefix + 1);

						mCached = mValue.Substring(0, prefix + 1);
						mobileHiddenInputPosition = cursorPosition;
						mobileHiddenInputCache = val;
						mobileHiddenInputCompositionString = string.Empty;
					}
		...
		if (mKeyboard != null)
		{
			...
			if (inputShouldBeHidden)
			{
				...
				if (text != "|")
				{
					...
					{
						// update text and compositionString
						if (mobileHiddenInputCache != text)
						{
							mobileHiddenInputCache = text;
							mobileHiddenInputCompositionString = string.Empty;

							// update selection
							mSelectionStart = mobileHiddenInputPosition;
							mSelectionEnd = mValue.Length;

							// replace selection
							if (label.supportEncoding)
							{
								string insert = text.Substring(1);
								StringBuilder sb = new StringBuilder();
								for (int i = 0; i < insert.Length; ++i)
								{
									char ch = insert[i];
									// Strip symbol character
									switch (ch)
									{
										case '$':
										case '{':
							...
							// update compositionString
							if (mLastIME != mobileHiddenInputCompositionString)
							{
								mSelectionEnd = string.IsNullOrEmpty(mobileHiddenInputCompositionString) ? mSelectionStart : mValue.Length + mobileHiddenInputCompositionString.Length;
								mLastIME = mobileHiddenInputCompositionString;
								UpdateLabel();
								ExecuteOnChange();
							}
						}
...
#elif UNITY_ANDROID
				if (text != mobileHiddenInputCache)
				{
					// update text and compositionString
					mobileHiddenInputCache = text;
					mobileHiddenInputCompositionString = string.Empty;
					...
					// re-insert
					mValue = mCached;
					mSelectionStart = mSelectionEnd = mValue.Length;
				...
				if (mKeyboard.done || !mKeyboard.active)
				{
					mobileHiddenInputCompositionString = string.Empty;
					mobileHiddenInputPosition = 0;
					mobileHiddenInputCache = string.Empty;
				}
			}
			...
			if (mKeyboard.done || !mKeyboard.active)
			{
				if (!mKeyboard.wasCanceled && !inputShouldBeHidden) Submit();
				mKeyboard = null;
				isSelected = false;
				mCached = "";
			}
	...
	protected void DoBackspace ()
	{
		if (!string.IsNullOrEmpty(mValue))
		{
			...
			{
				int pre = 0, next = 0;
				mSelectionEnd = label.GetCorrectCharacterIndex(GetProcessValue(), mSelectionEnd, out pre, out next);
				mSelectionStart = label.GetCorrectCharacterIndex(GetProcessValue(), mSelectionStart, out pre, out next);
				if (mSelectionStart == mSelectionEnd)
				{
					if (mSelectionStart < 1) return;
					int tail = 0;
					mSelectionEnd = label.GetCorrectTagIndex(GetProcessValue(), pre, out tail);
				}
				...
				if (!string.IsNullOrEmpty(mValue))
				{
						...
						if (label.supportEncoding)
						{
							...
							if (mSelectionStart == mSelectionEnd)
							{
								if (mSelectionStart >= mValue.Length) return true;
								int tail = 0;
								label.GetCorrectTagIndex(GetProcessValue(), next, out tail);
								mSelectionEnd = tail;
							}
			...
			case KeyCode.LeftArrow:
			{
				...
				if (!string.IsNullOrEmpty(mValue))
				{
						...
						else
						{
							...
							mSelectionEnd = label.GetCorrectCharacterIndex(GetProcessValue(), mSelectionEnd, out pre, out next);
							int tail = 0;
							pre = label.GetCorrectTagIndex(GetProcessValue(), pre, out tail);
							mSelectionEnd = Mathf.Max(pre, 0);
							if (!shift) mSelectionStart = mSelectionEnd;
							UpdateLabel();
						}
				}
			...
			case KeyCode.RightArrow:
			{
				...
				if (!string.IsNullOrEmpty(mValue))
				{
						...
						else
						{
							...
							int tail = 0;
							label.GetCorrectTagIndex(GetProcessValue(), next, out tail);
							mSelectionEnd = Mathf.Min(tail, mValue.Length);
							if (!shift) mSelectionStart = mSelectionEnd;
							UpdateLabel();
						}
				}
	...
	protected virtual void Insert (string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
			...
			if (!label.supportEncoding && characterLimit > 0 && sb.Length + rl >= characterLimit) break;
		...
		if (label.supportEncoding)
		{
			...
			// then correct index
			int pre = 0, next = 0;
			mSelectionEnd = mSelectionStart = label.GetCorrectCharacterIndex(GetProcessValue(), mSelectionStart, out pre, out next);
			int tail = 0;
			mSelectionEnd = mSelectionStart = label.GetCorrectTagIndex(GetProcessValue(), mSelectionStart, out tail);
		}
...
#endif
			int tail = 0;
			selectionEnd = label.GetCorrectTagIndex(mValue, GetCharUnderMouse(), out tail);
	...
	private string GetProcessValue()
	{
		if (label == null)
		{
			return string.Empty;
	...
	public void UpdateLabel ()
	{
		if (label != null)
		{
			...
			else
			{
...
#if MOBILE
				if (selected && inputShouldBeHidden) left += mobileHiddenInputCompositionString;
#endif
				...
				if (selected && label.overflowMethod == UISymbolLabel.Overflow.ClampContent && label.maxLineCount == 1)
				{
					...
					else
					{
						...
						{
							mDrawStart = offset;
							label.alignment = NGUISymbolText.Alignment.Right;
						}
					}
