...
static public class NGUISymbolText
{
	...
	static public bool useFakeSymbols = false;
	static public System.Func<string, int, int, BMSymbol> fakeSymbolHandler;
	static public float pixelSizeAdjustment = 1f;
	...
	static public void Update (bool request)
	{
		...
		useFakeSymbols = (fakeSymbolHandler != null && encoding && symbolStyle != SymbolStyle.None);
	...
	static public int GetCharacterWidth(char ch)
	{
		int width = 0;
	...
	static readonly Regex colReg = new Regex(@"(^\[[0-9a-fA-F]{6,8}\])|(^\[-\])", RegexOptions.Singleline);
	static readonly Regex emptyColsReg = new Regex(@"(\[[0-9a-fA-F]{6,8}\]\[-\])|(\[[0-9a-fA-F]{6,8}\]$)", RegexOptions.Singleline);
	static readonly Regex tagStartReg = new Regex(@"^\[([a-z]{1,5})([^\]]*)\]", RegexOptions.Singleline);
	static readonly Regex tagEndReg = new Regex(@"^\[/([a-z]+)\]", RegexOptions.Singleline);

	static EB.Collections.Stack<string> tags = new EB.Collections.Stack<string>(3);
	static EB.Collections.Stack<string> tagNames = new EB.Collections.Stack<string>(3);
	static EB.Collections.Stack<string> tagContents = new EB.Collections.Stack<string>(3);
	static EB.Collections.Stack<int> tagPos = new EB.Collections.Stack<int>(3);

	static public string ProfanityFilter(string text, System.Func<string, string> filterFunc)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
			...
			if (ParseSymbol(text, ref i))
			{
				...
				result += filterFunc(tmp) + text.Substring(j, i - j + 1);
				tmp = string.Empty;
				continue;
			}

			// See if there is a symbol matching this text
			BMSymbol symbol = useSymbols ? GetSymbol(text, i, imax) : null;
			symbol = symbol == null && useFakeSymbols ? fakeSymbolHandler(text, i, imax) : symbol;
			...
			{
				result += filterFunc(tmp) + symbol.sequence;
				tmp = string.Empty;

				i += symbol.sequence.Length - 1;
			}
		}
		result += filterFunc(tmp);

		return result;
	}

	static public int GetCorrectSymbolIndex(string text, int currentIndex, out int tail)
	{
		currentIndex = Mathf.Clamp(currentIndex, 0, text.Length);
		text = text.Insert(currentIndex, "\0");
		...
		while (i < textLength)
		{
			string search = text.Substring(i);

			Match startMat = tagStartReg.Match(search);
			if (startMat.Success)
			{
				if (!string.IsNullOrEmpty(tag))
				{
					...
					tagPos.Push(tagIndex);
				}

				tag = startMat.Value;
				tagName = startMat.Groups[1].Value;
				tagIndex = i;

				i += startMat.Index + startMat.Length;

				continue;
			}

			Match endMat = tagEndReg.Match(search);
			if (endMat.Success)
			{
				string endTagName = endMat.Groups[1].Value;
				if (endTagName != tagName)
				{
					...
					else
					{
						...
						{
							head = tagIndex;
							tail = i + endMat.Index + endMat.Length;

							head = head <= currentIndex ? head : head - 1;
							tail = tail <= currentIndex ? tail : tail - 1;
							return head;
						}
				...
				i += endMat.Index + endMat.Length;

				continue;
			}

			Match colMat = colReg.Match(search);
			if (colMat.Success)
			{
				i += colMat.Index + colMat.Length;
			...
			symbol = symbol == null && useFakeSymbols ? fakeSymbolHandler(text, i, textLength) : symbol;
		...
		UnityEngine.Debug.Assert(hit, "Hit nothing?");
		tail = text.Length;
		head = currentIndex;
		if (!string.IsNullOrEmpty(tag))
		{
	...
	static public string StripSymbol(string text, ref int currentIndex, int limit)
	{
		currentIndex = Mathf.Clamp(currentIndex, 0, text.Length);
		text = text.Insert(currentIndex, "\0");
		...
		while (i < textLength)
		{
			...
			if (endMat.Success)
			{
				...
				if (endTagName != tagName)
				{
					...
					if (tags.Count > 0)
					{
						string preTag = tags.Pop();
						string preContent = tagContents.Pop();
						string preTagName = tagNames.Pop();

						tagContent = colReg.Replace(tagContent, "");
						if (!string.IsNullOrEmpty(tagContent))
						{
							preContent += string.Format("{0}", tagContent);
						}

						tag = preTag;
						tagContent = preContent;
						tagName = preTagName;
					}
					else
					{
						tagContent = emptyColsReg.Replace(tagContent, "");
						if (!string.IsNullOrEmpty(tagContent))
						{
							result += string.Format("{0}", tagContent);
						}
				...
				else
				{
					...
					if (tags.Count > 0)
					{
						...
						if (!string.IsNullOrEmpty(tagContent))
						{
							preContent += string.Format("{0}{1}{2}", tag, tagContent, endTag);
						}
					...
					else
					{
						...
						if (!string.IsNullOrEmpty(tagContent))
						{
							result += string.Format("{0}{1}{2}", tag, tagContent, endTag);
						}
			...
			if (symbol != null)
			{
				...
				tagContent += symbol.sequence;
				i += symbol.length;

				continue;
			}

			elementWidth += GetCharacterWidth(text[i]);
			if (limit > 0 && elementWidth > limit)
			{
				// drop tags & break
				if (!string.IsNullOrEmpty(tag))
		...
		while (tags.Count > 0)
		{
			...
			string preContent = tagContents.Pop();
			tagContent = colReg.Replace(preContent, "") + tagContent;
		}
		result += tagContent;

		while (true)
		{
			string tmp = emptyColsReg.Replace(result, "");
			if (tmp == result)
			{
				break;
			}
	...
	static public void Adjust(BetterList<Vector3> verts, int indexOffset, float yOffset)
	{
		if (yOffset != 0)
		{
			for (int i = indexOffset; i < verts.size; ++i)
				verts.buffer[i].y += yOffset;
		}
	}
	...
	static public Vector2 CalculatePrintedSize (string text)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			float maxLineHeight = 0f, delta = 0f;
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (ch == '\n')
				{
					delta = Mathf.Max(maxLineHeight - finalLineHeight, 0);
					{
						y += delta;
						maxLineHeight = 0;
					}
				...
				else
				{
					...
					maxLineHeight = Mathf.Max(maxLineHeight, symbol.height * fontScale);
				}
			}
	...
	static BetterList<int> mIndexes = new BetterList<int>();
	static BetterList<int> mLengthes = new BetterList<int>();
	...
	static public int CalculateOffsetToFit(string text)
	{
		...
		for (int i = 0, imax = text.Length; i < imax; ++i)
		{
			...
			else
			{
				...
				mIndexes.Add(i);
				mLengthes.Add(symbol.sequence.Length);
				i += symbol.sequence.Length - 1;
				prev = 0;
			}
		}
		...
		int offset = mIndexes[currentCharacterIndex];
		if (currentCharacterIndex > 0)
		{
			// keep tags
			offset = mIndexes[currentCharacterIndex - 1] + mLengthes[currentCharacterIndex - 1];
		}
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool wrapLineColors, bool useEllipsis = false)
	{
		...
		for (; offset < textLength; ++offset)
		{
			...
			symbol = symbol == null && useFakeSymbols ? fakeSymbolHandler(text, offset, textLength) : symbol;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				delta = Mathf.Max(maxLineHeight - finalLineHeight, 0);
				{
					...
					Adjust(verts, adjustIndexOffset, -delta);
					adjustIndexOffset = verts.size;
				}
			...
			if (symbol != null)
			{
				...
				v0y = -(y + fontSize * fontScale + symbol.offsetY * fontScale);
				v1y = v0y + symbol.height * fontScale;
				...
				if (cols != null)
				{
					...
					{
						Color col = useFakeSymbols ? new Color(1f, 1f, 1f, 0f) : gc;
						for (int b = 0; b < 4; ++b) cols.Add(col);
					}
					else
					{
						Color col = Color.white;
						col.a = useFakeSymbols ? 0f : gc.a;
						for (int b = 0; b < 4; ++b) cols.Add(col);
					}
				}
	...
	static public void PrintApproximateCharacterPositions (string text, BetterList<Vector3> verts, BetterList<int> indices)
	{
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			else
			{
				...
				verts.Add(new Vector3(x, -y - halfSize));
				indices.Add(i + symbol.sequence.Length);
				i += symbol.sequence.Length - 1;
				prev = 0;
	...
	/// <summary>
	/// Print the caret and selection vertices.
	/// </summary>
	...
	static public void PrintCaretAndSelection (string text, int start, int end, BetterList<Vector3> caret, BetterList<Vector3> highlight)
	{
		...
		float maxLineHeight = 0f, delta = 0f; int hightlightAdjustIndexOffset = 0, caretAdjutIndexOffset = (caret != null) ? caret.size : 0;
		...
		for (; index < textLength; ++index)
		{
			...
			if (caret != null && !caretSet && caretPos <= index)
			{
				caretSet = true;
				float l = x - 1;
				float r = l + pixelSizeAdjustment;
				caret.Add(new Vector3(l, -y - fs));
				caret.Add(new Vector3(l, -y));
				caret.Add(new Vector3(r, -y));
				caret.Add(new Vector3(r, -y - fs));
			}
			...
			if (ch == '\n')
			{
				...
				if (caret != null && caretSet)
				{
					...
					delta = Mathf.Max(maxLineHeight - finalLineHeight, 0);
					{
						Adjust(caret, caretAdjutIndexOffset, -delta);
					}
					caret = null;
				}
				...
				if (highlight != null)
				{
					...
					delta = Mathf.Max(maxLineHeight - finalLineHeight, 0);
					{
						Adjust(highlight, hightlightAdjustIndexOffset, -delta);
						hightlightAdjustIndexOffset = highlight.size;
					}
				}
			...
			symbol = symbol == null && useFakeSymbols ? fakeSymbolHandler(text, index, textLength) : symbol;
			...
			if (w != 0f)
			{
				...
				index += (symbol != null ? symbol.sequence.Length - 1 : 0);

				maxLineHeight = Mathf.Max(maxLineHeight, symbol != null ? symbol.height * fontScale : finalLineHeight);
			}
		}
