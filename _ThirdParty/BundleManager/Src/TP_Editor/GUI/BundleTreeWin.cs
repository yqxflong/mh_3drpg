...
internal class BundleTreeWin : EditorWindow
{
			...
			EditorGUILayout.BeginHorizontal(EditorStyles.toolbar, GUILayout.ExpandWidth(true));
			{
				...
				if (GUI.Button(createBtnRect, "Create", EditorStyles.toolbarDropDown))
				{
					...
					if (m_Selections.Count <= 1)
					{
						...
						menu.AddItem(new GUIContent("AssetBundles From Selection"), false, CreateAssetBundlesFromSelection);
						menu.AddItem(new GUIContent("AssetBundles From Scene"), false, CreateAssetBundlesFromScene);
						menu.AddItem(new GUIContent("Folder Node"), false, CreateFolderNode);
					}
					else
					{
				...
				if (GUI.Button(buildBtnRect, "Build", EditorStyles.toolbarDropDown))
				{
					...
					menu.AddItem(new GUIContent("Build All Levels"), false, BuildAllLevels);
					menu.AddItem(new GUIContent("Build Level"), false, BuildLevel);
					menu.AddItem(new GUIContent("Clear"), false, ClearOutputs);
                    menu.AddItem(new GUIContent("Refresh"), false, RefreshAllBundle);
					menu.DropDown(buildBtnRect);
				}

				Rect deployBtnRect = GUILayoutUtility.GetRect(new GUIContent("Deploy"), EditorStyles.toolbarDropDown, GUILayout.ExpandWidth(false));
				if (GUI.Button(deployBtnRect, "Deploy", EditorStyles.toolbarDropDown))
				{
					GenericMenu menu = new GenericMenu();
					menu.AddItem(new GUIContent("PrepareBuildState"), false, PrepareBuildState);
					menu.AddItem(new GUIContent("Deploy Data"), false, DeployData);
					menu.AddItem(new GUIContent("Redeploy All Data"), false, RedeployData);
					menu.AddItem(new GUIContent("Deploy Data + StreamingAssets folder"), false, DeployDataAndStreamingAssetsFolder);
					menu.AddItem(new GUIContent("Redeploy All Data + StreamingAssets folder"), false, RedeployDataAndStreamingAssetsFolder);
					menu.AddItem(new GUIContent("Copy Data To StreamingAssets folder"), false, CopyDataToStreamingAssetsFolder);
					menu.AddItem(new GUIContent("Copy Video To StreamingAssets folder"), false, CopyVideoToStreamingAssetsFolder);
					menu.AddItem(new GUIContent("SaveBuildState"), false, SaveBuildState);
					menu.DropDown(deployBtnRect);
				}
	...
	bool EditProcess(Rect itemRect, string bundleName)
	{
		if (m_CurrentEditing == bundleName)
		{
			...
			{
				if (IsNameValid(m_EditString) || (m_EditString.StartsWith("+") && IsNameValid(m_EditString.Substring(1))))
					BundleManager.RenameBundle(bundleName, m_EditString);
				else
					EB.Debug.LogWarning("{0} is not valid for bundle name. Use only characters, numbers, _ and /", m_EditString);
	...
	void BuildLevel()
	{
		BuildHelper.BuildLevel();
	}

	void DeployData()
	{
		BuildHelper.BuildForDeploy(false);
		BuildHelper.DeployVideoToStreamingAssetsFolder();
	}

	void RedeployData()
	{
		BuildHelper.BuildForDeploy(true);
		BuildHelper.DeployVideoToStreamingAssetsFolder();
	}

	void DeployDataAndStreamingAssetsFolder()
	{
		BuildHelper.BuildForDeploy(false);
		BuildHelper.DeployDataToStreamingAssetsFolder();
		BuildHelper.DeployVideoToStreamingAssetsFolder();
	}

	void RedeployDataAndStreamingAssetsFolder()
	{
		BuildHelper.BuildForDeploy(true);
		BuildHelper.DeployDataToStreamingAssetsFolder();
		BuildHelper.DeployVideoToStreamingAssetsFolder();
	...
	void CopyDataToStreamingAssetsFolder()
	{
		BuildHelper.DeployDataToStreamingAssetsFolder();
		BuildHelper.DeployVideoToStreamingAssetsFolder();
	}

	void CopyVideoToStreamingAssetsFolder()
	{
		BuildHelper.DeployVideoToStreamingAssetsFolder();
	}
	...
	public static BundleTreeWin Instance { get; private set; }
	...
	void OnEnable()
	{
		Debug.Log("BundleTreeWin OnEnable");
		Instance = this;
	}

	void SaveBuildState()
	{
		BuildHelper.ProcessBuildState(true);
    }

	void PrepareBuildState()
	{
		BuildHelper.ProcessBuildState(false);
	}
}
