...
public class DownloadManager : MonoBehaviour 
{
	 ...
	 */ 
	public UnityWebRequest GetWWW(string url)
	{
		if(!ConfigLoaded)
			return null;
	...
	public float ProgressOfBundles(string[] bundlefiles)
	{
		...
		foreach(string bundlefile in bundlefiles)
		{
			...
			{
				EB.Debug.LogWarning("ProgressOfBundles only accept bundle files. {0} is not a bundle file.",bundlefile);
				continue;
			}
		...
		foreach(string bundleName in allInludeBundles)
		{
			...
			{
				EB.Debug.LogError("Cannot get progress of [{0}]. It's not such bundle in bundle build states list.", bundleName);		
				continue;
			}
			...
			if(processingRequest.ContainsKey(url))
				currentSize += (long)(processingRequest[url].www.downloadProgress * bundleSize);
	...
	IEnumerator Start() 
	{
        ...
        UnityWebRequest redirectWWW = UnityWebRequest.Get(formatUrl("BMRedirect.txt"));
		yield return redirectWWW;
		
		if(redirectWWW.isHttpError || redirectWWW.isNetworkError)
		{
			// Redirect download
			string downloadPathStr = BMUtility.InterpretPath(redirectWWW.downloadHandler.text, curPlatform);
        ...
        UnityWebRequest initDataWWW;

		if(bmUrl.offlineCache)
		{
			initDataWWW = UnityWebRequestAssetBundle.GetAssetBundle(bmDataUrl, (uint)(lastBMDataVersion + 1));
			...
			if(initDataWWW.error != null)
			{
				...
				initDataWWW = UnityWebRequestAssetBundle.GetAssetBundle(bmDataUrl, (uint)lastBMDataVersion);
				yield return initDataWWW;

				if(initDataWWW.error != null)
				{
					EB.Debug.LogError("Download BMData failed.\nError: {0}", initDataWWW.error);
					yield break;
				}
			}
			else
		...
		{
			initDataWWW = UnityWebRequest.Get(bmDataUrl);
		...
		TextAsset ta = DownloadHandlerAssetBundle.GetContent(initDataWWW).LoadAsset<TextAsset>("BundleData");
#else
        ...
        // Build States
#if UNITY_5 || UNITY_2018
        ta = DownloadHandlerAssetBundle.GetContent(initDataWWW).LoadAsset<TextAsset>("BuildStates");
#else
        ...
        // BMConfiger
#if UNITY_5 || UNITY_2018
        ta = DownloadHandlerAssetBundle.GetContent(initDataWWW).LoadAsset<TextAsset>("BMConfiger");
#else
		ta = initDataWWW.assetBundle.Load("BMConfiger") as TextAsset;
#endif
		bmConfiger = JsonMapper.ToObject< BMConfiger >(ta.text);

        DownloadHandlerAssetBundle.GetContent(initDataWWW).Unload(true);
		initDataWWW.Dispose();
		...
		foreach(WWWRequest request in processingRequest.Values)
		{
			if(request.www.error != null)
			{
				...
				else
				{
					...
					EB.Debug.LogError("Download {0} failed for {1} times.\nError: {2}",request.url,request.triedTimes , request.www.error);
				}
			}
	...
	void prepareDependBundles(string bundleName)
	{
		...
		foreach(string dependBundle in dependencies)
		{
			...
			if(succeedRequest.ContainsKey(dependUrl))
			{
				...
				var assetBundle = DownloadHandlerAssetBundle.GetContent(succeedRequest[dependUrl].www);
	...
	void download(WWWRequest request)
	{
		...
		if(isBundleUrl(request.url))
		{
			...
			{
				EB.Debug.LogError("Cannot download bundle [{0}]. It's not in the bundle config." , bundleName);
				return;
			}
	...
	List<string> getDependList(string bundle)
	{
		...
		{
			EB.Debug.LogError("Cannot find parent bundle [{0}], Please check your bundle config." , bundle);
			return res;
		}
	...
	BuildPlatform getRuntimePlatform()
	{
		...
		{
			EB.Debug.LogError("Platform {0} is not supported by BundleManager.",Application.platform);
			return BuildPlatform.Standalones;
		}
	}
		...
		{	
			...
			if(DownloadManager.instance.bmConfiger.useCache && bundleBuildState != null)
			{
				...
				if(DownloadManager.instance.bmConfiger.useCRC)
					www = UnityWebRequestAssetBundle.GetAssetBundle(url, (uint)bundleBuildState.version, bundleBuildState.crc);
				else 
#endif
					www = UnityWebRequestAssetBundle.GetAssetBundle(url, (uint)bundleBuildState.version);
			}
			else
