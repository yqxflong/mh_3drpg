...
namespace tar_cs
{
    ...
    public class TarReader
    {
        ...
        {
            //Debug.Log("tar stream position Read in: " + inStream.Position);
            int readBytes;
            byte[] read;
            while ((readBytes = Read(out read)) != -1)
            {
               //EB.Debug.Log("tar stream position Read while(...) : " + inStream.Position);
                dataDestanation.Write(read, 0, readBytes);
            }
          // EB.Debug.Log("tar stream position Read out: " + inStream.Position);
        }
        ...
        {
            //Debug.Log("tar stream position MoveNext in: " + inStream.Position);
            if (remainingBytesInFile > 0)
            {
                if (!skipData)
                {
            ...
            if(IsEmpty(bytes))
            {
                ...
                {
                    //Debug.Log("tar stream position MoveNext  out(false): " + inStream.Position);
                    return false;
                }
            ...
            //Debug.Log("tar stream position MoveNext  out(true): " + inStream.Position);
            return true;
        }
    }
}
