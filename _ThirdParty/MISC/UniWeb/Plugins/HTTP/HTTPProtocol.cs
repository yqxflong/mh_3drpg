		...
		public static void ReadChunks(Stream inputStream, Stream output, ref float progress)
		{
			...
			while (true)
			{
				...
				while (length > 0)
				{
					...
					if (count == 0)
					{
						throw new HTTPException(string.Format("HTTPProtocal ReadChunks timeout, length = {0} remaining = {1}", hexLength, length));
					}
...
#if BNICKSON_UPDATED
		public static void ReadChunksAsync(Stream inputStream, Stream output, ref float progress)
		{
			byte[] buffer = new byte[8192];
			while (true)
			{
				...
				while (length > 0)
				{
					var async = inputStream.BeginRead(buffer, 0, Mathf.Min (buffer.Length, length), null, null);
					if (!async.AsyncWaitHandle.WaitOne(System.TimeSpan.FromSeconds(HttpConnection.ReadTimeout)))
					{
						throw new System.Exception(string.Format("HTTPProtocal ReadChunksAsync BeginRead timeout, length = {0} remaining = {1}", hexLength, length));
					}
					var count = inputStream.EndRead(async);
			...
			if (int.TryParse(headers.Get("Content-Length"), out contentLength))
			{
				if (contentLength > 0)
				{
					...
					while (remaining > 0)
					{
						...
						if (count == 0)
						{
							throw new HTTPException(string.Format("HTTPProtocal ReadBody timeout, length = {0} remaining = {1}", contentLength, remaining));
						}
						remaining -= count;
						output.Write(buffer, 0, count);
...
#if BNICKSON_UPDATED
		public static void ReadBodyAsync(Stream inputStream, Stream output, Headers headers, bool strict, ref float progress)
		{
			// Read Body
			byte[] buffer = new byte[8192];
			int contentLength = 0;
			...
			if (int.TryParse(headers.Get("Content-Length"), out contentLength))
			{
				if (contentLength > 0)
				{
					...
					while (remaining > 0)
					{
						var async = inputStream.BeginRead(buffer, 0, buffer.Length, null, null);
						if (!async.AsyncWaitHandle.WaitOne(System.TimeSpan.FromSeconds(HttpConnection.ReadTimeout)))
						{
							throw new HTTPException(string.Format("HTTPProtocal ReadBodyAsync timeout, length = {0} remaining = {1}", contentLength, remaining));
						}
						var count = inputStream.EndRead(async);
