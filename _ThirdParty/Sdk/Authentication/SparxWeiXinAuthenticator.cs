...
namespace EB.Sparx
{
    public class WeiXinAuthenticator : Authenticator
    {
        ...
        public void Authenticate(bool silent, Action<string, object> callback)
        {
            if (silent)
            {
                if (mManager.GetPlatform() == TencentSDKManager.WXPlatform)
                {
            ...
            {
                mManager.Login(TencentSDKManager.WXPlatform, callback);
            }
        }
