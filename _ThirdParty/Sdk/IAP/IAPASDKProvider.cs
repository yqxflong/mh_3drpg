...
{
    public class ASDKProvider : Provider
    {
        ...
        public static ASDKProvider Create(Config config)
        {
            return new ASDKProvider(config);
        }

        private ASDKProvider(Config config)
        {
            mConfig = config;
        }
        ...
        {
            EB.Debug.Log("ASDKProvider.Complete: transaction {0} completed", transaction.transactionId);
        ...
        public void PurchaseItem(Item item)
        {
            ...
            {
                EB.Debug.LogWarning("ASDKProvider.PurchaseItem: get transaction not end");
                if (mConfig != null && mConfig.Verify != null)
                {
                    OnPayResult(EB.Sparx.AStatusCode.SUCCESS, string.Empty);
                }
            }

            /*if (mCurrent != null)
            {
                EB.Debug.LogWarning("ASDKProvider.PurchaseItem: current is purchasing transactionId = {0}", mCurrent.transactionId);
                return;
            }

            if (mCurrentItem != null)
            {
                EB.Debug.LogWarning("ASDKProvider.PurchaseItem: current is purchasing productId = {0}", mCurrentItem.productId);
                return;
            }*/
            ...
            {
                {"appId", SparxHub.Instance.ASDKProvider.AppId},
            ...
            EB.Sparx.Hub.Instance.WalletManager.BuyPayout(mCurrentItem, delegate (string error, IAP.Transaction transaction)
            {
                ...
                {
                    EB.Debug.LogWarning("ASDKProvider.PurchaseItem RequestBuyPayout GetCallback error = {0}", error);
                ...
                SparxHub.Instance.ASDKManager.Pay(mCurrentItem, transaction, OnPayResult);
            }, null);
        }

        private void OnPayResult(int code, object jsonOrder)
        {
            if (code == EB.Sparx.AStatusCode.SUCCESS)
            {
            ...
            else if (code == EB.Sparx.AStatusCode.FAIL)
            {
            ...
            else
            {
                ...
                if (code == EB.Sparx.AStatusCode.CHEAKING)
                {
        ...
        public void OnPayCallback(string transactionId)
        {
            if (mCurrent != null && mCurrentItem != null)
            {
                ...
                {
                    EB.Debug.Log("ASDKProvider.PurchaseItem: get pay callback from server");
                    if (mConfig != null && mConfig.Verify != null)
                    {
                        OnPayResult(EB.Sparx.AStatusCode.SUCCESS, string.Empty);
                    }
