...
namespace PixelCrushers.DialogueSystem {
	...
	public class ExtraDatabases : MonoBehaviour {
		...
		private void AddDatabase(DialogueDatabase database) {
			if (database != null) {
				if (DialogueDebug.LogInfo)EB.Debug.Log(string.Format("{0}: Adding database {1}", new object[] { DialogueDebug.Prefix, database.name }), this);
		...
		private void RemoveDatabase(DialogueDatabase database) {
			if (database != null) {
				if (DialogueDebug.LogInfo)EB.Debug.Log(string.Format("{0}: Removing database {1}", new object[] { DialogueDebug.Prefix, database.name }), this);
