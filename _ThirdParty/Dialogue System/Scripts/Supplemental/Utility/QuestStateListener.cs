...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class QuestStateListener : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            if (enabled)
            {
                if (DialogueDebug.LogInfo)EB.Debug.Log("Dialogue System: " + name + ": Listening for state changes to quest '" + questName + "'.", this);
                m_started = true;
        ...
        public void UpdateIndicator()
        {
            ...
            for (int i = 0; i < questStateIndicatorLevels.Length; i++)
            {
                ...
                {
                    if (DialogueDebug.LogInfo)EB.Debug.Log("Dialogue System: " + name + ": Quest '" + questName + "' changed to state " + questState + ".", this);
            ...
            for (int i = 0; i < questEntryStateIndicatorLevels.Length; i++)
            {
                ...
                {
                    if (DialogueDebug.LogInfo)EB.Debug.Log("Dialogue System: " + name + ": Quest '" + questName + "' entry " + questEntryStateIndicatorLevel.entryNumber + " changed to state " + questEntryState + ".", this);
