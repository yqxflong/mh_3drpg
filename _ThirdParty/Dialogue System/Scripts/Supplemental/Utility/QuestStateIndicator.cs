...
namespace PixelCrushers.DialogueSystem
{
    ...
    public class QuestStateIndicator : MonoBehaviour
    {
        ...
        {
            if (DialogueDebug.LogInfo)EB.Debug.Log("Dialogue System: " + name + ": SetIndicatorLevel(" + listener + ", " + indicatorLevel + ")", listener);
            ...
            for (int i = indicators.Length - 1; i >= 0; i--)
            {
                if (m_currentIndicatorCount[i].Count > 0)
                {
                    if (indicators[i] != null)
                    {
                        ...
                        if (DialogueDebug.LogInfo)EB.Debug.Log("Dialogue System: " + name + ": Activating GameObject associated with indicator level " + i, this);
                    }
                    break;
                }
            }
