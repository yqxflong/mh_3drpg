															 ...
															 List<GameObject> usedObjsToMesh) //Will be populated, is a subset of allObjsToMesh
		{
			...
			for (int i = 0; i < allObjsToMesh.Count; i++){
                ...
                for (int matIdx = 0; matIdx < sharedMaterials.Length; matIdx++){ //for each submesh
                    ...
                    if (_considerNonTextureProperties)
                    {
                        TextureBlender blender = FindMatchingTextureBlender(mat.shader.name);
                        if (blender != null && !(blender is TextureBlenderFallback)) {
                            setOfTexs.textureBlender = blender;
                        } else {
                            setOfTexs.textureBlender = new TextureBlenderFallback();
                            Debug.LogWarningFormat("Could not find a TextureBlender for shader '{0}'. Textures will be copied ignoring non-texture properties.", mat.shader.name);
                        }
                    }
                    */
					...
					if (setOfTexs2 != null){
                        ...
                        /*
                        if (considerNonTextureProperties && LOG_LEVEL >= MB2_LogLevel.trace)
                        {
                            Debug.Log(String.Format("Using TextureBlender {0} for material {1}", setOfTexs.textureBlender, matt.mat));
                        }
                        */
                    }
