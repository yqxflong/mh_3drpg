...
namespace EB.Sparx
{
	public class FacebookAPI : SparxAPIWithoutEndpoint
	{
		...
		public Coroutine Post(string uri, Hashtable parameters, System.Action<string, object> callback)
		{
			return Coroutines.Run(_Graph("POST", uri, parameters, callback));
		}

		public Coroutine Get(string uri, Hashtable parameters, System.Action<string, object> callback)
		{
		...
		IEnumerator _Graph(string method, string uri, Hashtable parameters, System.Action<string, object> callback)
		{
			var url = "https://graph.facebook.com"+uri;

			if (parameters == null)
			{
				parameters = Johny.HashtablePool.Claim();
			}
