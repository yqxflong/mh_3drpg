...
namespace EB
{
	...
	class BitmapData
	{
		...
		public BitmapData(Texture2D texture, float scale )
		{
			_text = texture;
		
			height = Mathf.RoundToInt(texture.height*scale);
			width = Mathf.RoundToInt(texture.width*scale);
		...
		{	
			var u = Mathf.Clamp01( (float)x * _invWidth);
			var v = Mathf.Clamp01( (float)y * _invHeight);
			
			return _text.GetPixelBilinear(u,v);
		}
	}
	...
	class JPGEncoder
	{
		...
		*/
		public byte[] GetBytes()
		{
			if(!isDone)
			{
				EB.Debug.LogError("JPEGEncoder not complete, cannot get bytes!");
				return null;
			}
		...
		public JPGEncoder(Texture2D texture, float quality) :
			this(texture,quality,1.0f)
		{
			
		}
		
		public JPGEncoder(Texture2D texture, float quality, float scale)
		{	
			// save out texture data to our own data structure
			image = new BitmapData(texture, scale);
