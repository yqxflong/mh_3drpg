...
namespace EB.Replication
{
	...
	{		
		...
		public delegate Object InstantiateMethodDelegate( Object prefab, Vector3 position, Quaternion rotation ,string pathName);
		...
		// for load level
		private static Hashtable _doneLoading = Johny.HashtablePool.Claim();
		...
		{
			UpdateRate = 15;
			RegisterRPC(LoadLevelRPC, (System.Action<uint,uint,string,bool,bool>)_LoadLevelRPC);
			RegisterRPC(LoadLevelCompleteRPC, (System.Action<uint>)_LoadLevelCompleteRPC );
			RegisterRPC(InstantiateRPC, (System.Action<uint,uint,Object,Vector3,Quaternion>)_InstantiateRPC );
			RegisterRPC(TransferSceneOwnershipRPC, (System.Action<uint,uint>)_TransferSceneOwnershipRPC);
			RegisterRPC(TransferOwnershipRPC, (System.Action<ViewId,uint,uint>)_TransferOwnershipRPC);
			RegisterRPC(DestroyRPC, (System.Action<uint,uint>)_DestroyRPC);
			RegisterRPC(BroadcastRPC, (System.Action<string>)_BroadcastMessageRPC);
		}
...
// 		}
		...
		public static Object Instantiate( Object prefab, Vector3 position, Quaternion rotation ,string pathName =null)
		{
			var nextId = _localN;
			_localN   += MaxNetworkViewsPerObject;
			...
			return _Instantiate(_localP, nextId, prefab, position, rotation, pathName); 
		}
		...
		static Object _Instantiate( uint ownerId, uint nextId, Object prefab, Vector3 position, Quaternion rotation,string pathName=null)
		{
		//	EB.Debug.Log("_Instantiate {0}, {1}, {2}", ownerId, nextId, prefab ); 
			...
			if (prefab != null)
			{
				...
				{
					r = InstantiateMethod( prefab, position, rotation ,pathName);
				}
				else
				{
		...
		static IEnumerator _LoadLevelCoroutine( uint ownerId, uint nextId, string name, bool additive, bool force )
		{
			...
			{
				yield return Assets.LoadLevel( name, true);
			}
			else
			{
				yield return Assets.LoadLevel( name );
