...
{	
	...
	public class View : MonoBehaviour
	{
		...
		// just a place to store the last hash of the serialize view for delta compression
		...
		public void Serialize( BitStream bs )
		{
			...
			else if( observed is Rigidbody )
			{
				...
				var angularVel = GetComponent<Rigidbody>().angularVelocity;
				...
				if ((GetComponent<Rigidbody>().constraints & RigidbodyConstraints.FreezeRotationX) != 0)
				{
					bs.Serialize(ref rot.x);
					bs.Serialize(ref angularVel.x);
				}
					
				if ((GetComponent<Rigidbody>().constraints &  RigidbodyConstraints.FreezeRotationY) != 0)
				{
					bs.Serialize(ref rot.y);
					bs.Serialize(ref angularVel.y);
				}
					
				if ((GetComponent<Rigidbody>().constraints &  RigidbodyConstraints.FreezeRotationZ) != 0)
				{
