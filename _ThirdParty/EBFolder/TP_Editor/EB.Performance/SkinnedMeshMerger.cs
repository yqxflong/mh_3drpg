...
public class SkinnedMeshMerger : MonoBehaviour {
	...
	public static List<GameObject> Merge(MeshFilter[] meshFilters, string savedName, string folderName) {
		...
		foreach(MeshFilter meshFilter in meshFilters)
		{
			...
			if(!skinnedMeshInfos.ContainsKey(meshFilter.GetComponent<Renderer>().sharedMaterial))
			{
				skinnedMeshInfos[meshFilter.GetComponent<Renderer>().sharedMaterial] = new List<MeshFilter>();
			}
			skinnedMeshInfos[meshFilter.GetComponent<Renderer>().sharedMaterial].Add(meshFilter);
		}
		...
		foreach(KeyValuePair<Material, List<MeshFilter>> skinnedMeshInfo in skinnedMeshInfos)
		{
			...
			for( int i = 0; i < skinnedMeshInfo.Value.Count; i++ ) 
			{
				...
				DestroyImmediate(meshFilter.GetComponent<Renderer>());
