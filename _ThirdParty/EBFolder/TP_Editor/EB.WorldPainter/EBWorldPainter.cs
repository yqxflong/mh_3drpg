...
public class EBWorldPainter : EditorWindow 
{
	...
	private static string DataPathForCurrentScene()
	{
		...
		string[] scenePathComponents = scenePath.Split(slash);
		string sceneName = scenePathComponents[scenePathComponents.Length - 1].Split('.')[0];
		int built_level_name_index = sceneName.IndexOf("Built");
		bool is_built_data = built_level_name_index != -1;
		if(is_built_data)
		{
			sceneName = sceneName.Remove(built_level_name_index, sceneName.Length - built_level_name_index);
		}
		if (sceneName == string.Empty)
			return null;
		return "Assets" + slash + "GM" + slash + "GameAssets" + slash + "Environment" + slash + sceneName + slash + "Prefabs" + slash  + "WorldPainterData" +".prefab";
	}
