...
public class EBWorldPainterData : MonoBehaviour
{
	...
	//[System.Serializable]
    // disable warning because this is unused
    public class Region
	{
		public int id;
		public List<Point> points;
	...
	public void Unpack()
	{
		...
		foreach(Region region in regions)
		{
			...
			for (int i = region.visibleRegions.Count - 1; i >= 0; --i)
			{
				bool found = false;
				foreach(Region r in regions)
				{
					if (r.id == region.visibleRegions[i].id)
					{
						region.visibleRegions[i] = r;
						found = true;
						break;
					}
				}
				if (!found)
				{
					Debug.LogWarning("Can't hook back up a visible region!");
					region.visibleRegions.RemoveAt(i);
				}
			}
	...
	public bool SplitRegionAcrossPoints(Region region, Point p1, Point p2)
	{
		...
		splitRegion.visibleRegions.Add(region);
		splitRegion.visibleRegions.AddRange(region.visibleRegions);
		region.visibleRegions.Add(splitRegion);
	...
	public bool RemoveLine(Point p1, Point p2)
	{
		...
		foreach(Region region in region2.visibleRegions)
		{
			if (!newRegion.visibleRegions.Contains(region))
			{
				newRegion.visibleRegions.Add(region);
			}
		}
		...
		foreach(Region region in regions)
		{
			...
			if (region.visibleRegions.Contains(region1))
			{
				region.visibleRegions.Remove(region1);
				contained = true;
			}
			if (region.visibleRegions.Contains(region2))
			{
				region.visibleRegions.Remove(region2);
				contained = true;
			}
			if (contained)
			{
				region.visibleRegions.Add(newRegion);
			}
		}
